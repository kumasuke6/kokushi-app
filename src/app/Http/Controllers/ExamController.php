...
class ExamController extends Controller
{
    public function showQuestions(ShowQuestionRequest $request)
    {
        ...
        list($questions, $seed) = $questionModel->getQuestions($request->seed, $request->subjectIds, $request->qRandom);
        ...
        // choiceをランダムにする処理
        $resultChoices = array();
        if (!is_null($request->choiceRandom)) {
        ...
        return view('exam', ['questions' => $questions, 'choices' => $resultChoices, 'aryAnswers' => $aryAnswer, 'seed' => $seed, 'examNumber' => $request->examNumber]);
    }
}
